...
KERNEL_FLAGS = [
    f"-DLDG_NUM_BLOCKS={_env_int('LDG_NUM_BLOCKS', 96)}",
    f"-DLDG_BLOCK_SIZE={_env_int('LDG_BLOCK_SIZE', 512)}",
    # 1.7B talker has vocab=3072 (vs 0.6B's 151936). Counter-intuitively,
    # KEEP a high block count (1184) — RTX 5090 has 170 SMs, so we need many
    # blocks for SM occupancy even though each block now has fewer vocab rows
    # to handle. Empirically: 1184 blocks → 503 tok/s; 24 blocks → 143 tok/s.
    f"-DLDG_LM_NUM_BLOCKS={_env_int('LDG_LM_NUM_BLOCKS', 1184)}",
